...
def ogranicz_wiersz (ktory, mozliwe_wiersze, plansza):
...
def ogranicz_kolumne(ktora, mozliwe_kolumny, plansza):
    kolumna = []
    for i in range(m):
...
def szukaj (plansza):
    ...
    for i in range(n):
        ...
        if not sprawdz_ustawienie(kolumna, x[i]):
            zle_kolumny.add(i)
    pop = []
    while not end(plansza) and plansza != pop:
        pop = plansza.copy()
        for i in range(m):
            if i in zle_wiersze:
                ogranicz_wiersz(i, mozliwe_wiersze, plansza)
            if sprawdz_ustawienie(plansza[i], y[i]):
                zle_wiersze -= set([i])
        for i in range(n):
            if i in zle_kolumny:
                ogranicz_kolumne(i, mozliwe_kolumny, plansza)
            kolumna = []
            for j in range(m):
                kolumna.append(plansza[j][i])
...
def wypisz_do_pliku (plansza):
    plansza = obroc_plansze(plansza)
    for i in range(n):
        for j in range(m):
            if plansza[i][j] == '1':
