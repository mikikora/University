...
class Player:
    ...
    def rate_move_oponent(self, your_game, your_player):
        depth = 0
        q = Queue()
        my_moves = your_game.moves(your_player)
        for e in my_moves:
            for i in range(len(my_moves[e])):
                #print(self.game.pieces, file=debug)
                new_piece_board = [list() + e for e in your_game.pieces]
                g = Jungle(your_player, new_piece_board)
        ...
        while not q.empty():
            parent, act_game, act_player, vic = q.pop()
            if vic == '0 won' and your_player == 0:
                res.append((parent, 100+act_game.rate_state(0)))
            elif vic == '1 won' and your_player == 1:
                res.append((parent, 100 + act_game.rate_state(1)))
            elif vic == '0 won' and your_player == 1:
                res.append((parent, -100 + act_game.rate_state(1)))
            elif vic == '1 won' and your_player == 0:
                res.append((parent, -100 + act_game.rate_state(0)))
            else:
                res.append((parent, act_game.rate_state(your_player)))
    ...
    def rate_move(self):
        ...
        for e in my_moves:
            for i in range(len(my_moves[e])):
                ...
                vic = g.do_move(e, my_moves[e][i])
                q.push(((e, i), g, g.player, vic, depth))
        while not q.empty():
            parent, act_game, act_player, vic, depth = q.pop()
            if depth > 0 or vic is not None:
                #print('cos sie dzieje')
                ended.append((parent, act_game, vic))
            else:
                if act_player != self.player:
                    he_chose_a, he_chose_b = self.rate_move_oponent(act_game, act_player)
                    vic = act_game.do_move(he_chose_a, he_chose_b)
                    q.push((parent, act_game, act_game.player, vic, depth + 1))
                else:
                    my_moves = act_game.moves(act_player)
                    for e in my_moves:
                        for i in range(len(my_moves[e])):
                            new_piece_board = [list() + e for e in act_game.pieces]
                            g = Jungle(act_player, new_piece_board)
                            vic = g.do_move(e, my_moves[e][i])
                            q.push((parent, g, g.player, vic, depth))
        #print('jestem tu')
        for parent, act_game, vic in ended:
        ...
        r = max(res, key=lambda x: x[1])[0]
        result = (r[0], self.game.moves(self.player)[r[0]][r[1]])
        return result
