...
jungle:
  ready_timeout: 5 # seconds
  move_timeout: 55  # seconds
  game_timeout: 6000 # seconds
'''
...
class Jungle:
    ...
    def update(self, player, move_string):
        ...
        move = tuple(int(m) for m in move_string.split())
        if len(move) != 4:
            raise WrongMove('czyli tutaj')
        possible_moves = self.moves(player)
        if not possible_moves:
            if move != (-1, -1, -1, -1):
                raise WrongMove('ale tutaj')
            move = None
        else:
            move = ((move[0], move[1]), (move[2], move[3]))
            if move not in possible_moves:
                print(move, possible_moves, sep='\n\n')
                #input()
                raise WrongMove('tutaj')
        self.do_move(move)
...
class Player(object):
    ...
    def _reader(self):
        ...
        while proc.poll() is None:
            prompt = proc.stdout.readline().decode()
            if VERBOSE > 1:
    ...
    def expect(self, prompt, block=True, timeout=None):
        ret = str(self.get(block, timeout))
