...
#;(define (update x v xs)
  (cond
    [(null? xs)
     (error x "unknown identifier :(")]
    [(eq? (mcar (mcar xs)) x)
     (begin (set-mcdr! (mcar xs) (mlist v))
            xs)]
    [else
     (mcons (mcar xs) (update x v (mcdr xs)))]))
...
(define (op-to-proc x)
  (lookup x (mlist (mlist '+ +)
                   ...
                   (mlist '<= <=)
                   (mlist '= =)
                   (mlist '!= (lambda (x y) (not (= x y))))
                   (mlist '&& (lambda (x y) (and x y)))
                   (mlist '|| (lambda (x y) (or x y)))
                   )))
...
(define (env-add x v env) (mcons (mlist x v) env))
