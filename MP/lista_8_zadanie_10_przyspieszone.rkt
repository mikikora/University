...
(define (test-performance)
  (let-values
      (((r1 cpu1 real1 gc1) (time-apply find-prime-using-WHILE (list 1111111)))
       ...
       ((r3 cpu3 real3 gc3) (time-apply factorial              (list 10000)))
       ((r4 cpu4 real4 gc4) (time-apply factorial-nativ        (list 10000))))
    (begin
...
(define gcd-in-WHILE
    (while (op '> (variable 'b) (const 0))
           (comp
            (if-stm (op '< (variable 'a) (variable 'b))
                    (var-block 'temp (variable 'b)
                               (comp (assign 'b (variable 'a))
                                     (assign 'a (variable 'temp))))
                    (skip))
            (var-block 'pom (op '- (variable 'a) (variable 'b))
                       (comp (assign 'a (variable 'b))
                             (assign 'b (variable 'pom))))))
    )
