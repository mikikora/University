...
(define (clause-set-add rc rc-set)
  #;(define (eq-cl? sc);;to juz nie jest potrzebne procedura too-easy? sprawdza rowniez to co procedura eq-cl? 
    (and (equal? (res-clause-pos rc)
                 (res-clause-pos sc))
         (equal? (res-clause-neg rc)
                 (res-clause-neg sc))))
  (define (too-easy? sc);;nowy predykat
    (subsumes? sc rc))
  (define (remove-too-easy rc rest);;nowa funkcja
    (if (null? rest)
        null
        (if (subsumes? rc (car rest))
            (remove-too-easy rc (cdr rest))
            (cons (car rest) (remove-too-easy rc (cdr rest))))))
  (define (add-to-stopped sset)
    (let ((procd  (cadr  sset))
          (toproc (caddr sset)))
      (cond
       [(null? procd) (list 'stop (list rc) '())]
       [(or (memf too-easy? procd);;tu zmiana
            (memf too-easy? toproc));;tu tez
        sset]
       [else (let ((new-procd (remove-too-easy rc procd)))
               (if (null? new-procd)
                   (list 'stop rc (remove-too-easy rc toproc))
                   (list 'stop new-procd (cons rc (remove-too-easy rc toproc)))))];;nowy else
       #;[else (list 'stop procd (remove-too-easy rc toproc))])))
  (define (add-to-running rset)
    (let ((pd  (second rset))
          (tp  (third  rset))
          (cc  (fourth rset))
          (rst (fifth  rset)))
      (if (or (memf too-easy? pd);;tu zmiana
              (memf too-easy? tp);;tu tez
              (too-easy? cc);;tu tez
              (memf too-easy? rst));tu tez
          rset
          (let ([new-pd (remove-too-easy rc pd)]
                [new-tp (remove-too-easy rc tp)]
                [new-rst (remove-too-easy rc rst)])
            (cond [(and (null? new-tp) (subsumes? rc cc))
                   (if (null? new-pd)
                       (list 'stop (list rc) new-rst)
                       (list 'stop new-pd (cons rc new-rst)))]
                  [(and (not (subsumes? rc cc)) (null? new-tp)) (list 'stop (cons cc new-tp) (cons rc new-pd))]
                  [(subsumes? rc cc) (list 'stop (append new-pd new-tp) (cons rc new-rst))]
                  [else (if (null? new-pd)
                            (list 'run (list rc) new-tp cc new-rst)
                            (list 'run new-pd new-tp cc new-rst))]));;nowy warunek
          #;(list 'run pd tp cc (remove-too-easy rc rst)))))
...
;;tutaj moje procedury
(define (subsumes? c1 c2)
    (define (in a b)
      (if (null? a)
          true
          (and (not (not (member (car a) b))) (in (cdr a) b))))
...
#;(define (clause-set-map proc lst)
  (define (stop-set-map proc lst)
    )
  (define (run-set-map proc lst)
    )
  (if (eq? 'stop lst)
      (stop-set-map proc lst)
      (run-set-map proc lst)))
